document.addEventListener('DOMContentLoaded', () => {
    ...
    let square = Array.from(document.querySelectorAll('.grid div')) //Array.from collects everything from the div and put them in index  
    ...
    const zTetromino = [
      ...
      function moveRight() {
          ...
          if(current.some(index => square[currentPosition + index].classList.contains('taken'))){
              currentPosition -= 1 //if run into an occupied array, push the current position back
          }
          draw()
      }
      ...
      function rotate() {
          undraw()
          currentRotation ++
          if (currentRotation === current.length) { //back to original shape
              currentRotation = 0
          }
          current = theTetrominoes[random][currentRotation]
          draw()
      ...
      function addScore() {
        for (let i = 0; i < 199; i +=width) {
          ...
          if(row.every(index => square[index].classList.contains('taken'))) {
            ...
            square = squaresRemoved.concat(square)
